...
for i, p in enumerate(params):
    ...
    for j in range(len(params)):
        if j < i:   # lower triangle: vertical + horizontal
...
# --- Legend ---
handles = [
    mlines.Line2D([], [], color='steelblue', lw=2, label='blackjax-ns'),
...
axes.iloc[0, 0].legend(handles=handles, loc='lower center',
                       bbox_to_anchor=(3.5, 1.05), ncol=3, frameon=False)

fig.savefig('triangle_4s.pdf', bbox_inches='tight')
print("Saved triangle_4s.pdf")
