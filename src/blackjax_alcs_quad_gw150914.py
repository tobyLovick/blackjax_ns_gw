...
# ---------------------------------------------------------------------------
# ALCS per-bin marginalisation via MAP-centred Gauss-Hermite quadrature
# ---------------------------------------------------------------------------
...
def loglikelihood_fn(params):
    ...
    for det in detectors:
        ...
        log_L = log_L + jnp.dot(_alcs_bins(B, jnp.log(det.psd), p["tau"], log_two_df_over_pi), det.mask)
    return log_L
...
samples.to_csv(f"blackjaxns_alcs_quad_gw150914{suffix}.csv")
with open(f'blackjaxns_alcs_quad_gw150914{suffix}_final_state.pkl', 'wb') as f:
    pickle.dump(final_state, f)
with open(f'blackjaxns_alcs_quad_gw150914{suffix}_timings.pkl', 'wb') as f:
