...
column_to_label = {
    ...
    "psi": r"$\psi$", "phase_c": r"$\phi_c$", "alpha_0": r"$\alpha_0$",
}
...
def log_uniform_transform_alpha0(u):
    # log-uniform on [3, 1000]: log(alpha_0) ~ Uniform(log 3, log 1000)
...
def loglikelihood_fn(params):
    """Inverse-gamma analytic noise-marginalised GW log-likelihood."""
...
with open('blackjaxns_invg_nlive1400_final_state.pkl', 'wb') as f:
...
samples.to_csv("blackjaxns_invg_nlive1400.csv")
# Save timings from progress bar
with open('blackjaxns_invg_nlive1400_timings.pkl', 'wb') as f:
