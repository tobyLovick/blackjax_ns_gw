...
# ---------------------------------------------------------------------------
# ALCS per-bin Laplace marginalisation
# ---------------------------------------------------------------------------
def _alcs_single_bin(B_i, log_S0_i, tau, log_two_df_over_pi):
    ...
    for _ in range(5):
        ...
        s   = s - f / fp
    log_f = (log_two_df_over_pi
             - 2.0 * s
             - B_i * jnp.exp(-2.0 * s)
             - 0.5 * jnp.log(2.0 * jnp.pi * tau_sq)
             - (s - s0) ** 2 / (2.0 * tau_sq))
...
with open('blackjaxns_alcs_gw150914_final_state.pkl', 'wb') as f:
...
samples.to_csv("blackjaxns_alcs_gw150914.csv")
with open('blackjaxns_alcs_gw150914_timings.pkl', 'wb') as f:
